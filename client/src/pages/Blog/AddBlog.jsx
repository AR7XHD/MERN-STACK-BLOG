...
const AddBlog = () => {
  ...
  const response = useFetch(`${getEnv("VITE_BASE_URL")}/category/all-category`, {
    method: "GET",
    headers: {
  ...
  // include content field with min length
  ...
  const onSubmit = async (values) => {
    try {
      ...
      const response = await fetch(`${getEnv("VITE_BASE_URL")}/blog/add-blog`, {
        method: "POST",
        credentials: "include",
        body: payload,
      });
      ...
      if (!response.ok) {
        showToast("error", result.message || "Failed to add blog");
        return;
      }

      showToast("success", result.message || "Blog added");
      navigate(RouteBlog);
    } catch (err) {
