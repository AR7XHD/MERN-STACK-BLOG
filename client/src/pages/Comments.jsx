...
const Comments = () => {
  ...
  const response = useFetch(`${getEnv("VITE_BASE_URL")}/comment/get-comments`, {
    method: "GET",
    headers: {
...
const handleDelete = async (id) => {
  ...
  const response = await fetch(`${getEnv("VITE_BASE_URL")}/comment/delete-comment/${id}`, {
    method: "DELETE",
    headers: {
        "Content-Type": "application/json",
    },
        ...
        <TableBody>
         {data?.comments?.map((item, index) => (
            <TableRow key={index}>
              ...
              <TableCell>{item.blog.title}</TableCell>
